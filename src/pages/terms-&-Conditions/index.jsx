...
const TermsConditions = () => {
  ...
  const handleSave = async () => {
    try {
      const res = await instance.put(termsApi, {
        terms: state.text,
      });
    } catch (err) {
      // Error handling
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await instance.get(termsApi);
        setTerms(res.data.terms); // Set the data in the state
        dispatch({ type: ActionTypes.EDIT_TEXT, payload: res.data.terms }); // Set the text in the reducer
      } catch (err) {
        // Error handling
        console.log("An error occurred:", err.message);
      }
